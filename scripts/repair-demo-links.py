...
for u in s['users']:
 ...
 if not found:
  call('/admin/realms/dda/users','POST',{'id':old,'username':u['email'],'email':u['email'],'emailVerified':True,'enabled':True,'firstName':u['firstName'],'lastName':u['lastName'],'federatedIdentities':[{'identityProvider':'dex','userId':subject,'userName':u['name']}]})
  found=call('/admin/realms/dda/users?email='+urllib.parse.quote(u['email'])+'&exact=true')
 assert len(found)==1
