...
In the Keycloak console, select **dda → Users → select user → Groups → Join Group**. Add the user to the appropriate group, then sign out of Open WebUI and use **Continue with DDA SSO** again. Adding both groups gives administrator access. Removing `openwebui-admins` while retaining `openwebui-users` demotes the user on the next SSO sign-in. Membership in neither group denies a new SSO login. Avoid direct `webui-*` role grants or other group grants that would bypass this policy.
...
The bootstrap realm creates both groups; demo imports assign the five fixtures to `openwebui-users` and the dedicated administrator to `openwebui-admins`. Production imports omit demo identities. Newly provisioned identities automatically join `openwebui-users`; administrators control later memberships. `sync-keycloak.py` migrates existing demo direct grants once and preserves later group membership decisions. Run `verify-group-inheritance.py` only in the local demo: it temporarily promotes testuser05, tests demotion and denial, then restores membership.
...
## Default group for new identities

`openwebui-users` is configured in **Realm settings → User registration → Default groups**. New Keycloak identities, including users created on their first brokered login, receive this group and inherit ordinary WebUI access. Administrators can change membership afterward; later logins do not undo their changes. `openwebui-admins` is never assigned by default. The migration renames the legacy group in place, retaining its ID, memberships and role mappings. The source realm uses `defaultGroups: ["/openwebui-users"]`; the live reconciler uses the Keycloak default-group API.
